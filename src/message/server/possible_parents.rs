...
impl MessageHandler<ServerOperation> for PossibleParentsHandler {
    ...
    fn handle(&self, message: &mut Message, sender: Sender<ServerOperation>) {
        ...
        debug!("Received {} possible parents from server.", number_of_parents);
        ...
        for _ in 0..number_of_parents {
            ...
            let host = format!("{}.{}.{}.{}", ip_parts[3], ip_parts[2], ip_parts[1], ip_parts[0]);
            let port = message.read_int32();

            // 1. Acknowledge the parent by sending their IP back to the server
            let parent_ip_message = MessageFactory::build_parent_ip_message(ip_parts.clone());
            sender.send(ServerOperation::SendMessage(parent_ip_message)).unwrap();
